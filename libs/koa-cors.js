...
module.exports = async (ctx, next) => {
  ctx.set('Access-Control-Allow-Origin', 'http://127.0.0.1:8889'); //允许来自所有域名请求(不携带cookie请求可以用*，如果有携带cookie请求必须指定域名)
  ...
  ctx.set('Access-Control-Allow-Headers', 'x-requested-with, accept, origin, content-type, authtoken'); // 字段是必需的。它也是一个逗号分隔的字符串，表明服务器支持的所有头信息字段.
  ...
  // ctx.set('Access-Control-Expose-Headers'); // 需要获取其他字段时，使用Access-Control-Expose-Headers，
  // getResponseHeader('authtoken')可以返回我们所需的值
  /*
